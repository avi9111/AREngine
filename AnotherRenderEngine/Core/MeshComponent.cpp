...
void MeshComponent::InitBuffer()
{
	...
	for (UINT index = 0; index < mModelList.size(); ++index)
	{
		...
		for (int i = 0; i < (int)mModelData->mMeshList.size(); ++i)
		{
			...
			auto device = GDirectxCore->Graphics()->D3D()->GetDevice();
			device->CreateBuffer(&vertexBufferDesc, &vertexData, &mMesh.mVertexBuffer);
			...
			device->CreateBuffer(&indexBufferDesc, &indexData, &mMesh.mIndexBuffer);
		}
